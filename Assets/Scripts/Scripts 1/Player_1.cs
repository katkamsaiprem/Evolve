...
public class Player_1 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            speed *= -1f;
            Evolution_2_AudioManager.Instance.PlaySound(_moveClip);
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            GameManager1.Instance.UpdateScore();
            Evolution_2_AudioManager.Instance.PlaySound(_winClip);
            return;
        }
        if (collision.CompareTag(Constants.Tags.OBSTACLE))
        {
            GameManager1.Instance.EndGame();
            Evolution_2_AudioManager.Instance.PlaySound(_loseClip);
